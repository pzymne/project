...
export function getEmptyHex(length: number): string {
  return pad("0x", { size: length });
}

export function convertNumberToHex(num: number | bigint, length: number): Hex {
...
export function convertStringToHex(str: string): Hex {
  return toHex(`0x${Buffer.from(str).toString("hex")}`);
}

export function convertBooleanToHex(bool: boolean): Hex {
  return bool ? "0x01" : "0x00";
}

export function getRandomHex(length: number): Hex {
