...
import {
  ...
  SpokeRewardsV2GasToken__factory,
...
describe("SpokeRewardsV2GasToken contract (unit tests)", () => {
  ...
  async function deploySpokeFixture() {
    ...
    const spokeToken = await new SpokeRewardsV2GasToken__factory(user).deploy(
      admin.address,
      bridgeRouter,
      hubChainId,
      hubAddress,
  ...
  describe("Fund", () => {
    it("Should receive token", async () => {
      const { user, spokeToken } = await loadFixture(deploySpokeFixture);

      // fund
      const amount = BigInt(10e18);
      const fund = await spokeToken.connect(user).fund({ value: amount });
      const balance = await ethers.provider.getBalance(spokeToken);
